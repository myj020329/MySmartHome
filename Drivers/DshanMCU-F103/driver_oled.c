...
//让指定页的像素朝着设置的方向按设置的频率水平滚动且会将像素的行地址进行偏移offset这么多个单位
...
//设置OLED在水平地址模式或垂直地址模式下像素显示的起始行地址和结束行地址
...
//设置OLED在水平地址模式或垂直地址模式下像素显示的起始页地址和结束页地址
...
//4. 硬件配置功能函数 
//设置OLED从第line行开始显示，即将默认的0行偏移至line那一行
...
//显示一个字符
//输入参数：x --> x坐标(0~15)y --> y坐标(0~7)c -->   显示的字符
...
//显示一个字符串
//输入参数：x --> x坐标(0~15)y --> y坐标(0~7)str -->   显示的字符串
...
//刷新OLED的区域
//输入参数：col     - OLED的列,取值范围0~127row - OLED的行,取值范围0~63width - 宽度heigh - 高度,必须是8的整数倍
